...
EncodedTable<T> getDemographicTable(const double scaleFactor, sqlite3* sqlite_db, Engine& engine) {
    auto S = demographicSize(scaleFactor);

    Vector<uT> u_pid(S);
    engine.populateLocalRandom(u_pid);
    Vector<T> pid(S);
    pid = u_pid % demographicSize(scaleFactor);

    std::vector<std::string> schema = {"[pid]"};
    EncodedTable<T> table = engine.template secret_share_table<T>({pid}, schema);
...
template <typename T, typename Engine>
EncodedTable<T> getDiagnosisTable(const double scaleFactor, sqlite3* sqlite_db, Engine& engine) {
    auto S = diagnosisSize(scaleFactor);

    Vector<uT> u_code(S);
    engine.populateLocalRandom(u_code);
    Vector<T> code(S);
    code = u_code % CODE_TYPES;

    Vector<uT> u_diag(S);
    engine.populateLocalRandom(u_diag);
    Vector<T> diag(S);
    diag = u_diag % DIAG_TYPES;

    std::vector<std::string> schema = {"[code]", "[diag]"};
    EncodedTable<T> table = engine.template secret_share_table<T>({code, diag}, schema);
...
template <typename T, typename Engine>
EncodedTable<T> getMedicationTable(const double scaleFactor, sqlite3* sqlite_db, Engine& engine) {
    ...
    Vector<uT> u_med(S);
    engine.populateLocalRandom(u_med);
    Vector<T> med(S);
    med = u_med % MED_TYPES;

    std::vector<std::string> schema = {"[pid]", "[code]", "[med]"};
    EncodedTable<T> table = engine.template secret_share_table<T>({pid, code, med}, schema);
...
int main(int argc, char** argv) {
    ...
    auto sf = engine.getArg<float>("test-size", "r", 0.1);
    ...
    auto Demographic = getDemographicTable<T>(sf, sqlite_db, engine);
    auto Diagnosis = getDiagnosisTable<T>(sf, sqlite_db, engine);
    auto Medication = getMedicationTable<T>(sf, sqlite_db, engine);

...
EncodedTable<T> getCohortTable(const double scaleFactor, sqlite3* sqlite_db, Engine& engine) {
...
EncodedTable<T> getDiagnosisTable(const double scaleFactor, sqlite3* sqlite_db, Engine& engine) {
    ...
    EncodedTable<T> table = engine.template secret_share_table<T>({pid, diag}, schema);
...
int main(int argc, char** argv) {
    ...
    auto Cohort = getCohortTable<T>(sf, sqlite_db, engine);

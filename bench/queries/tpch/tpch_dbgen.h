...
class TPCDatabase {
   protected:
    ...
    Vector<T> randomColumn(size_t size, T min_val, T max_val) {
        ...
        Vector<uT> uvec(size);
        engine.populateLocalRandom(uvec);
   ...
   public:
    ...
    TPCDatabase(double sf, Engine& engine) : scaleFactor(sf), sqlite_db(nullptr), engine(engine) {}

    TPCDatabase(double sf, sqlite3* sqlite_db, Engine& engine)
        : scaleFactor(sf), sqlite_db(sqlite_db), engine(engine) {}
    ...
    EncodedTable<T> getCustomersTable() {
        ...
        EncodedTable<T> table = engine.template secret_share_table<T>(
            {custkey, name, address, nationKey, phone, cntrycode, acctbal, acctbal, mktsegment,
             comment},
            schema);
    ...
    EncodedTable<T> getLineitemTable(bool proactive_sharing = false) {
        ...
        if (pid == 0) {
            ...
            for (int p = 1; p < engine.getNumParties(); p++) {
                engine.comm0()->sendShare((int64_t)lineItemsSize, p);
            }
        } else {
            engine.comm0()->receiveShare((int64_t&)lineItemsSize, -pid);
        }
        ...
        EncodedTable<T> table = engine.template secret_share_table<T>(data, schema);
    ...
    EncodedTable<T> getRegionTable() {
        ...
        EncodedTable<T> table =
            engine.template secret_share_table<T>({regionKey, name, comment}, schema);
    ...
    EncodedTable<T> getNationTable() {
        ...
        EncodedTable<T> table =
            engine.template secret_share_table<T>({nationKey, name, regionKey, comment}, schema);
    ...
    EncodedTable<T> getPartTable() {
        ...
        EncodedTable<T> table = engine.template secret_share_table<T>(
            {partkey, brand, container, type, size, name}, schema);
    ...
    EncodedTable<T> getSupplierTable() {
        ...
        EncodedTable<T> table = engine.template secret_share_table<T>(
            {suppkey, nationkey, acctbal, name, address, phone, comment}, schema);
    ...
    EncodedTable<T> getPartSuppTable() {
        ...
        EncodedTable<T> table = engine.template secret_share_table<T>(
            {suppkey, partkey, supplyCost, supplyCost, availQty}, schema);
    ...
    EncodedTable<T> getPartTableSecretFlow(int num_rows) {
        ...
        EncodedTable<T> table = engine.template secret_share_table<T>({partKey, type}, schema);
    ...
    EncodedTable<T> getOrdersTableSecretFlow(int num_rows) {
        ...
        EncodedTable<T> table =
            engine.template secret_share_table<T>({orderKey, orderPriority}, schema);

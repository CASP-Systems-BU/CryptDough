...
int main(int argc, char** argv) {
    ...
    ////////////////////// DB Initialize ///////////////////////
    ////////////////////////////////////////////////////////////
    auto db = TPCDatabase<T, Engine>(sf, sqlite_db, engine);
...

    {
        ...
        if (pid == T1PartyID) {
            for (int j = 1; j < engine.getNumParties(); j++) {
                engine.comm0()->sendShare(i, j);
            }
        } else {
            engine.comm0()->receiveShare(i, T1PartyID - pid);
        }
...

    {
        ...
        if (pid == T2PartyID) {
            ...
            engine.comm0()->receiveShare(i, T2PartyID - pid);
        }
    ...
    EncodedTable<T> L2 = engine.secret_share_table<T>(DataT1, SchemaT1, T1PartyID);
    EncodedTable<T> O2 =
        engine.secret_share_table<T>(DataT2, SchemaT2, T2PartyID);  // TODO: make party sharing = 1
    ...
    auto tj = O2.uu_join(L2, {"[OrderKey]"},
                         ...
                         {.trim_invalid = false}, cdough::SortingProtocol::BITONICMERGE);
    ...
    for (int s = 0; s <= MAX_SHIPMODE; s++) {
        ...
        A sel_a = engine.secret_share_a(sel, T1PartyID);

        ASharedVector<T> h_count = tj.asASharedVector("HighLineCount") * sel_a;
        h_count.prefix_sum();
        h_count.tail(1);
        lineHigh[s] = h_count.open()[0];

        ASharedVector<T> l_count = tj.asASharedVector("LowLineCount") * sel_a;
        l_count.prefix_sum();
        l_count.tail(1);

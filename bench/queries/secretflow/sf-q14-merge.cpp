...
int main(int argc, char** argv) {
    ...
    EncodedTable<T> LineItem2 = engine.secret_share_table<T>(DataT1, SchemaT1, T1PartyID);
    EncodedTable<T> Part2 = engine.secret_share_table<T>(DataT2, SchemaT2, T2PartyID);
    ...
    // Convert to 64-bits
    ASharedVector<int64_t> kp(Join.size(), engine);
    kp = Join.asASharedVector("KeyedPromoRevenue");
    ASharedVector<int64_t> pr(Join.size(), engine);

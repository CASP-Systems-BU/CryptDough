...
int main(int argc, char** argv) {
    ...
    ASharedVector<int> a(TOTAL_SIZE, engine), b(TOTAL_SIZE, engine);

    for (size_t x = 1024; x <= TOTAL_SIZE; x <<= 1) {
        engine.setBatchSize(x);
    ...
    if (pID == 0) {
        for (int p = 1; p < engine.getNumParties(); p++) {
            engine.comm0()->sendShare((int64_t)best_batch, p);
        }
    } else {
        engine.comm0()->receiveShare((int64_t&)best_batch, -pID);
    }
    ...
    if (pID == 0) {
        ...
        if (outFile.is_open()) {
            ...
            outFile << "# protocol: " << STRING(COMPILED_MPC_PROTOCOL_NAMESPACE) << " ("
                    << engine.getNumParties() << "PC)\n";
            outFile << "# threads: " << engine.get_num_threads() << "\n";

...
std::vector<cdough::Vector<int>> build_town_table(int n) {
    std::vector<cdough::Vector<int>> res = {
        cdough::Vector<int>(NUM_TOWNS), cdough::Vector<int>(NUM_TOWNS), cdough::Vector<int>(NUM_TOWNS),
        cdough::Vector<int>(NUM_TOWNS), cdough::Vector<int>(NUM_TOWNS),
    };
...
std::vector<cdough::Vector<int>> build_citizens(int n) {
    std::vector<cdough::Vector<int>> res = {
        cdough::Vector<int>(n),
        cdough::Vector<int>(n),  // Income
        cdough::Vector<int>(n),
        cdough::Vector<int>(n),  // Cars
        cdough::Vector<int>(n),
        cdough::Vector<int>(n),  // House Sqft
        cdough::Vector<int>(n),
        cdough::Vector<int>(n),  // Children
        cdough::Vector<int>(n),
    };
...
void run_join_test(EncodedTable<int> towns, EncodedTable<int> citizens, int num_joins,
    ...
    EngineRef engine = towns.engine;

    std::vector<std::tuple<std::string, std::string, AggregationSelector<A, B>>> Agg;

    for (int i = 0; i < num_joins; i++) {
        Agg.push_back({town_schema[i + 1], town_schema[i + 1], cdough::aggregators::copy<A>});
    }

    for (int i = 0; i < num_aggs; i++) {
        Agg.push_back(
            {citizen_schema[2 * i + 1], citizen_schema[2 * i + 2], cdough::aggregators::sum<A>});
    }
...
void micro_uniq_join(Engine& engine) {
    ...
    for (int b = min_exp; b <= max_exp; b++) {
        ...
        EncodedTable<int> t1 =
            engine.template secret_share_table<int>({k1, a1, b1}, {"[K]", "A", "B"});

        auto k2 = Vector<int>(R);
        auto c2 = Vector<int>(R);

        EncodedTable<int> t2 = engine.template secret_share_table<int>({k2, c2}, {"[K]", "C"});

        single_cout("Start total size=" << total_size);
        ...
        for (int i = 0; i < repeat; i++) {
            ...
            single_cout("  semi: un=" << un << " uu=" << uu << "  " << std::fixed
        ...
        for (int i = 0; i < repeat; i++) {
            ...
            auto speedup = (double)un / uu;
            single_cout("  full: un=" << un << " uu=" << uu << "  " << std::fixed
                                      << std::setprecision(2) << speedup << "x");
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    if (run_uniq_join_micro) {
        ...
        for (int s = MIN_TABLE_EXPONENT; s <= MAX_TABLE_EXPONENT; s++) {
            ...
            EncodedTable<int> citizens = engine.secret_share_table(citizen_data, citizen_schema);

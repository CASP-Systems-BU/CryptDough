...
// Balanced-cluster mixed-effects logistic regression, Laplace approximated,
// from the logistic-regression branch (tasks 0001-0003).
//
// This is a different data layout from regression.h, not an older version of
// it. Here each cluster is its own ClusterGroup with its own vectors, which is
// the natural form when the clusters are balanced and the whole model fits in
// one objective. regression.h instead holds the analysis pipeline's flat
// layout, where every cluster lives in one padded table and the per-cluster
// sums come from segmented scans -- necessary there because real patients have
// a ragged number of encounters. Both are kept: the flat form scales to ragged
// data, and this one is the simpler reference that
// playground/secure-logistic-regression.cpp exercises directly.

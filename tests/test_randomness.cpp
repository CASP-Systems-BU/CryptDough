...
void test_local_prg_randomness(Engine& engine) {
    ...
    auto localPRG = engine.rand0()->localPRG;
...
void helper_check_group_generation_correct(int num_parties, int group_size,
    ...
    cdough::ProtocolBase::generateAllCombinations(parties, partial_combination, group_size, groups);
...
void test_3pc_common_prg_correctness(Engine& engine) {
    auto rank = engine.getPartyID();
    int test_size = cdough::random::AESPRGAlgorithm::MAX_AES_QUERY_BYTES;
    ...
    for (int relative_rank : relative_ranks) {
        ...
        auto common_prg = engine.rand0()->commonPRGManager->get(relative_rank);
    ...
    engine.comm0()->exchangeShares(shared_with_next, remote, +1, -1);
...
void test_4pc_common_prg_correctness(Engine& engine) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (relative_rank == 1) {
            engine.comm0()->exchangeShares(common_vec, remote1, -1, -1);
            engine.comm0()->exchangeShares(common_vec, remote2, -2, -2);
        } else if (relative_rank == 2) {
            engine.comm0()->exchangeShares(common_vec, remote1, -1, -1);
            engine.comm0()->exchangeShares(common_vec, remote2, +1, +1);
    ...
    for (std::set<int> group : engine.getGroups()) {
        ...
        if (rank == lowestRank) {
            ...
            for (int otherRank : group) {
                ...
                Vector<T> remote(test_size);
                engine.comm0()->exchangeShares(randomness, remote, relative_rank, relative_rank);
...
void test_zero_sharing_generator_correctness(Engine& engine) {
    ...
    // generate the values
    auto zeroSharingGenerator = engine.rand0()->zeroSharingGenerator;
    ...
    for (int i = 1; i < num_parties; i++) {
        ...
        engine.comm0()->exchangeShares(my_shares_arithmetic, other_shares_a, send_party,
                                       recv_party);
        engine.comm0()->exchangeShares(my_shares_binary, other_shares_b, send_party, recv_party);
...
int main(int argc, char** argv) {
    ...
    // unsigned
    test_local_prg_randomness<uint32_t>(engine);
    test_local_prg_randomness<uint64_t>(engine);
    // signed
    ...
    if (num_parties == 3) {
        test_3pc_common_prg_correctness<int32_t>(engine);
        test_3pc_common_prg_correctness<int64_t>(engine);
    } else if (num_parties == 4) {
        test_4pc_common_prg_correctness<int32_t>(engine);
        test_4pc_common_prg_correctness<int64_t>(engine);
    }
    ...
    test_common_prg_correctness_groups<int32_t>(engine);
    test_common_prg_correctness_groups<int64_t>(engine);
    if (pID == 0) std::cout << "Common Group Randomness...OK" << std::endl;

    // test random zero sharing
    test_zero_sharing_generator_correctness<int32_t>(engine);
    test_zero_sharing_generator_correctness<int64_t>(engine);
    if (pID == 0) std::cout << "Zero Sharings...OK" << std::endl;

    // test group random zero sharing
    test_zero_sharing_generator_groups<int32_t>(engine);
    test_zero_sharing_generator_groups<int64_t>(engine);
    if (pID == 0) std::cout << "Group Zero Sharings...OK" << std::endl;
#else
    single_cout("MPC_PROTOCOL_SPDZ_2K_NPC is defined. Skipping test_randomness.");
#endif

...
void test_multi_key(Engine& engine) {
    ...
    // clang-format off
    std::vector<cdough::Vector<int>> c1 = {
        {1, 1, 2, 3, 4},
        {1, 2, 4, 6, 7},
        Vector<int>(5)};
    // clang-format on
    EncodedTable<int> t1 = engine.secret_share_table(c1, {"[K1]", "[K2]", "[D]"});

    std::vector<cdough::Vector<int>> c2 = {{1, 1, 1, 3, 4, 4, 1, 1, 4, 5},
                                        {2, 1, 1, 6, 7, 7, 1, 2, 7, 8},
                                        {0, 1, 1, 2, 3, 5, 8, 3, 1, 4},
                                        Vector<int>(10)};
    EncodedTable<int> t2 = engine.secret_share_table(c2, {"[K1]", "[K2]", "[D2]", "C"});
...
void test_valid(Engine& engine) {
    auto pid = engine.getPartyID();
    // clang-format off
    std::vector<cdough::Vector<int>> pk_data = {
        {1, 2, 3, 4},
        {100, 200, 300, 400},
        {1, 0, 1, 0}};
    // clang-format on
    EncodedTable<int> P = engine.secret_share_table(pk_data, {"[K]", "[D2]", "[_VALID]"});
    P.filter(P["[_VALID]"]);

    // clang-format off
    std::vector<cdough::Vector<int>> fk_data = {
        { 1,  1,  1,  1,  2,  3,  3,  3,  3,  5,  5},
        {10, 20, 30, 32, 35, 40, 50, 60, 65, 70, 80},
        { 1,  1,  0,  1,  1,  1,  0,  0,  1,  1,  0},
    };
    // clang-format on
    EncodedTable<int> F = engine.secret_share_table(fk_data, {"[K]", "[Data]", "[_VALID]"});
    F.filter(F["[_VALID]"]);
...
void check_simple(Engine& engine) {
    ...
    std::vector<cdough::Vector<int>> c1 = {{1, 2}, {12345, 67890}};
    EncodedTable<int> t1 = engine.secret_share_table(c1, {"[UID]", "[Zipcode]"});
    t1.tableName = "LOC";

    std::vector<cdough::Vector<int>> c2 = {{2, 2}, {40, -60}};
    EncodedTable<int> t2 = engine.secret_share_table(c2, {"[UID]", "[Amount]"});
    t2.tableName = "TXN";
    ...
    auto tj =
        t1.inner_join(t2, {"[UID]"},
                      {
                          {"[Zipcode]", "[Zipcode]", cdough::aggregators::copy<BSharedVector<int>>},
                      });
    // tj may trim
    ...
    // clang-format off
    std::vector<cdough::Vector<int>> truth = {
...
void test_normal_behavior(Engine& engine) {
    ...
    std::vector<cdough::Vector<int>> c1 = {{1, 2, 3, -5, 10},
                                        {12345, 67890, 11111, 98989, 22222},
                                        {18, 22, 82, -10, 0},
                                        {1111, 2222, 3333, 1040, 7777}};
    EncodedTable<int> t1 = engine.secret_share_table(c1, {"[UID]", "[Zip]", "Age", "Amount"});
    t1.tableName = "User";

    // clang-format off
    std::vector<cdough::Vector<int>> c2 = {
    ...
    // clang-format on
    EncodedTable<int> t2 = engine.secret_share_table(c2, {"[UID]", "Amount", "[To]"});
    t2.tableName = "Txn";
...

    {
        auto tj = t1.inner_join(t2, {"[UID]"},
                                {
                                    {"[Zip]", "[Zip]", cdough::aggregators::copy<B>},
                                });
...

    {
        auto tj = t1.inner_join(t2, {"[UID]"},
                                {
                                    {"[Zip]", "[Zip]", cdough::aggregators::copy<B>},
                                    {"Age", "Age", cdough::aggregators::copy<A>},
                                });
...

    {
        auto tj = t1.inner_join(t2, {"[UID]"},
                                {
                                    {"Amount", "COUNT", cdough::aggregators::count<A>},
                                    {"Amount", "SUM_AMT", cdough::aggregators::sum<A>},
                                    {"Age", "Age", cdough::aggregators::copy<A>},
                                    {"[Zip]", "[Zip]", cdough::aggregators::copy<B>},
                                });
...
void test_outer(Engine& engine) {
    ...
    EncodedTable<int> t1 = engine.secret_share_table(c1, {"[K]", "[D]", "A"});

    std::vector<cdough::Vector<int>> c2 = {
        {222, 999, 666, 333},
        {1, 2, 3, 4},
    };
    EncodedTable<int> t2 = engine.secret_share_table(c2, {"[K]", "D2"});
...
void test_unique(Engine& engine) {
    ...
    {
        std::vector<cdough::Vector<int>> d1 = {
        ...
        // clang-format off
        std::vector<cdough::Vector<int>> d2 = {
            {  2,   3,   4,   5,   6,   9,  12,  14},
            {111, 222, 333, 444, 555, 888, 777, 999},
        };
        // clang-format on

        EncodedTable<int> t1 = engine.secret_share_table(d1, {"[K]", "A", "[Z]"});
        EncodedTable<int> t2 = engine.secret_share_table(d2, {"[K]", "B"});
        ...
        print_table(op, engine.getPartyID());
    ...
    EncodedTable<int> t1 = engine.secret_share_table(d1, {"[K1]", "[K2]", "A", "Q"});
    EncodedTable<int> t2 = engine.secret_share_table(d2, {"[K1]", "[K2]", "B", "W"});
...
int main(int argc, char** argv) {
    ...
    check_simple(engine);
    test_normal_behavior(engine);
    test_multi_key(engine);
    test_valid(engine);
    ...
    test_semi_anti_join(engine);

    test_unique(engine);
#else
    single_cout("MPC_PROTOCOL_SPDZ_2K_NPC is defined. Skipping test_join.");
#endif

...
void test_oprf_correctness(int test_size, Engine& engine) {
    ...
    BSharedVector<__int128_t> result_0(inputs.size(), engine);
    BSharedVector<__int128_t> result_1(inputs.size(), engine);
...
template <typename Engine>
void test_permutation_correlations(Engine& engine) {
    auto pID = engine.getPartyID();
    auto commonPRGManager = engine.rand0()->commonPRGManager;
...
int main(int argc, char** argv) {
    ...
    test_oprf_correctness(1000, engine);
    single_cout("OPRF Correctness...OK");

    test_permutation_correlations(engine);

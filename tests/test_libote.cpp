...
// Don't conflict with cdough's libOTe instance
...
void test_silent_vole_loopback(u64 n, Engine& engine) {
    auto L = std::numeric_limits<std::make_unsigned_t<F>>::digits;
    ...
    bool isServer = (engine.getPartyID() == 0);

    auto sock = cp::asioConnect(engine.comm0()->host_prefix + TESTING_ADDRESS, isServer);
 ...
 */
template <typename F, typename Engine>
void test_silent_ot_loopback(u64 n, Engine& engine) {
    assert(n % OT_BLOCK_SIZE == 0);
    auto L = std::numeric_limits<std::make_unsigned_t<F>>::digits;
...
template <typename F, typename Engine>
void test_silent_ot_chosen(u64 n, Engine& engine) {
    auto L = std::numeric_limits<std::make_unsigned_t<F>>::digits;
...
void test_softspoken(u64 n, Engine& engine) {
    bool isRecv = (engine.getPartyID() == 0);
    ...
    PRNG prng(sysRandomSeed());
    auto sock = cp::asioConnect(engine.comm0()->host_prefix + TESTING_ADDRESS, isRecv);
...
int main(int argc, char** argv) {
    ...
    test_silent_vole_loopback<int8_t>(test_size, engine);
    test_silent_vole_loopback<int32_t>(test_size, engine);
    test_silent_vole_loopback<int64_t>(test_size, engine);

    // 2PC tests of Silent OT
    test_silent_ot_loopback<int8_t>(test_size, engine);
    test_silent_ot_loopback<int32_t>(test_size, engine);
    test_silent_ot_loopback<int64_t>(test_size, engine);

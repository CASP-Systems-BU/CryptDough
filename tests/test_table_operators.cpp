...
EncodedTable<T> create_test_table(int column_count, Engine& engine) {
    ...
    return engine.secret_share_table(columns, schema);
}

template <typename T>
void assert_table_schema(EncodedTable<T>& table, size_t column_count) {
    EngineRef engine = table.engine;
...
void test_add_columns(Engine& engine) {
    ...
    EncodedTable<T> table = create_test_table<T>(1, engine);
...
void test_delete_columns(Engine& engine) {
    ...
    EncodedTable<T> table = create_test_table<T>(6, engine);
...
void test_resize(Engine& engine) {
    ...
    EncodedTable<T> table = engine.template secret_share_table<T>(
        {Vector<T>(n_rows, 99), Vector<T>(n_rows, 77)}, {"A", "B"});
...
int main(int argc, char** argv) {
    ...
    test_add_columns<int>(engine);
    test_delete_columns<int>(engine);
    test_project<int>(engine);
    test_resize<int>(engine);

...
void test_bitonicMerge(int test_size, Engine& engine) {
    ...
    cdough::Vector<T> v(v3);

    BSharedVector<T> v_secret = engine.secret_share_b(v, 0);
    cdough::operators::bitonic_merge(v_secret);
    auto v_opened = v_secret.open();

    EncodedTable<T> table = engine.template secret_share_table<T>({v}, {"[V]"});
    table.sort({{"[V]", SortOrder::ASC}}, cdough::SortingProtocol::BITONICMERGE);
...
void test_quicksort(int test_size, Engine& engine) {
    ...
    BSharedVector<T> nums = engine.secret_share_b(v, 0);
    nums.shuffle();
    Vector<T> shuffled = nums.open();

    BSharedVector<T> b1 = engine.secret_share_b(shuffled, 0);
    BSharedVector<T> b2 = engine.secret_share_b(shuffled, 0);
    BSharedVector<T> b1_reversed = engine.secret_share_b(shuffled, 0);
    BSharedVector<T> b2_reversed = engine.secret_share_b(shuffled, 0);

    // sort b1 and apply the sorting permutation to b2
    auto permutation = cdough::operators::quicksort(b1);
    ...
    auto reversed_permutation = cdough::operators::quicksort(b1_reversed, SortOrder::DESC);
...
void test_pairwise(int test_size, Engine& engine) {
    ...
    auto input = engine.secret_share_b(v, 0);

    input.shuffle();
    cdough::operators::pairwise_sort(input);
    auto asc_open = input.open();

    input.shuffle();
    cdough::operators::pairwise_sort(input, SortOrder::DESC);
...
//         cdough::operators::oblivious_apply_elementwise_perm(b, permutation);
...
void test_radix_sort(int test_size, Engine& engine) {
    ...
    // sort b1 and apply the sorting permutation to b2
    auto permutation = cdough::operators::radix_sort(b1);
    cdough::operators::oblivious_apply_elementwise_perm(b2, permutation);

    // sort b1_reversed in reverse and apply the sorting permutation to b2_reversed
    auto reversed_permutation = cdough::operators::radix_sort(b1_reversed, SortOrder::DESC);
    oblivious_apply_elementwise_perm(b2_reversed, reversed_permutation);
...
void test_valid_bit_sorting(int test_size, Engine& engine) {
    ...
    BSharedVector<int> b1 = engine.secret_share_b(v, 0);
    BSharedVector<int> b1_non_sort = engine.secret_share_b(v, 0);
    BSharedVector<int> b2 = engine.secret_share_b(v, 0);
    BSharedVector<int> b2_non_sort = engine.secret_share_b(v, 0);

    auto perm1 = cdough::operators::radix_sort(b1, SortOrder::ASC, 1);
    auto perm2 = cdough::operators::radix_sort(b2, SortOrder::DESC, 1);

    cdough::operators::oblivious_apply_elementwise_perm(b1_non_sort, perm1);
    cdough::operators::oblivious_apply_elementwise_perm(b2_non_sort, perm2);
...
// **************************************** //
template <typename Engine>
void test_table_sort_single(int num_rows, int num_columns, cdough::SortingProtocol protocol,
                            Engine& engine) {
    // generate a table
    ...
    // open
    std::vector<cdough::Vector<int>> opened1 = table1.open();
    std::vector<cdough::Vector<int>> opened2 = table2.open();
    ...
    assert(cdough::random::PermutationManager::get()->size() == 0);
    assert(cdough::random::PermutationManager::get()->size_pairs() == 0);
}
...
// **************************************** //
template <typename Engine>
void test_table_sort_multi(int num_rows, int num_columns, cdough::SortingProtocol protocol,
                           bool test_tiebreak_explicit, Engine& engine) {
...
void test_odd_even_merge(int test_size, Engine& engine) {
    ...
    BSharedVector<int> v = engine.secret_share_b(nums, 0);
    v.shuffle();

    // sort each half of the list
    cdough::Vector<int> shuffled = v.open();
    ...
    BSharedVector<int> v2 = engine.secret_share_b(vec_to_merge, 0);

    cdough::operators::odd_even_merge(v2);
    auto result = v2.open();
    for (int i = 0; i < test_size; i++) {
        assert(result[i] == i);
    }
...
int main(int argc, char** argv) {
    ...
    test_bitonicMerge<int>(TEST_SIZE, engine);
    single_cout("Bitonic Merge...OK");
    test_bitonicMerge<int64_t>(TEST_SIZE, engine);
    single_cout("Bitonic Merge 64...OK");

    test_quicksort<int>(TEST_SIZE, engine);
    single_cout("Quicksort...OK");
    test_quicksort<int64_t>(TEST_SIZE, engine);
    single_cout("Quicksort 64...OK");

    test_pairwise<int64_t>(1024, engine);
    test_pairwise<int64_t>(60000, engine);
    single_cout("Pairwise 64...OK");

    // test_gen_bit_perm(10000);
    // single_cout("GenBitPerm...OK");

    test_radix_sort<int>(TEST_SIZE, engine);
    single_cout("Radix Sort...OK");
    test_radix_sort<int64_t>(TEST_SIZE, engine);
    single_cout("Radix Sort 64...OK");

    test_valid_bit_sorting(TEST_SIZE * 10, engine);
    single_cout("Valid Bit Sort...OK");

    single_cout("TS RS...");
    test_table_sort_single(1024, 6, cdough::SortingProtocol::RADIXSORT, engine);
    single_cout("TS QS...");
    test_table_sort_single(1024, 6, cdough::SortingProtocol::QUICKSORT, engine);
    single_cout("Table Sort (Single Sort Column)...OK");

    single_cout("TS Multi RS...");
    test_table_sort_multi(256, 8, cdough::SortingProtocol::RADIXSORT, false, engine);
    test_table_sort_multi(256, 8, cdough::SortingProtocol::RADIXSORT, true, engine);
    single_cout("TS Multi QS...");
    test_table_sort_multi(256, 8, cdough::SortingProtocol::QUICKSORT, false, engine);
    test_table_sort_multi(256, 8, cdough::SortingProtocol::QUICKSORT, true, engine);
    single_cout("Table Sort (Multiple Sort Columns)...OK");

    test_odd_even_merge(16384, engine);
    single_cout("Odd-Even Merge...OK");

    single_cout("Quadratic Sort...OK");
#else
    single_cout("MPC_PROTOCOL_SPDZ_2K_NPC is defined. Skipping test_sorting.");
#endif

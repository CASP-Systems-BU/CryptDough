 ...
 * ```
 *
 * This example demonstrates cdough's relational analytics engine.
 *
...
int main(int argc, char** argv) {
    ...
    if (pID == 0) {
        ...
        engine.populateLocalRandom(SSN_t);
        // SSNs have 9 digits
        SSN_t %= MAX_SSN;

        engine.populateLocalRandom(Amount);
        // Positive amounts only
        Amount %= MAX_AMOUNT;

        // Duplicates? Could ensure uniqueness by contract, or use oblivious
        // distinct
        engine.populateLocalRandom(SSN_p);
        SSN_p %= MAX_SSN;
        engine.populateLocalRandom(Age);
        // 'U' here enforces unsigned mod
        Age %= 100U;
        engine.populateLocalRandom(State);
        State %= NUM_STATES;
    }

    EncodedTable<int> T1 = engine.secret_share_table<int>({SSN_p, State, Age}, schema1);
    EncodedTable<int> T2 = engine.secret_share_table<int>({SSN_t, Amount}, schema2);

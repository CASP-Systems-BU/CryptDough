...
int main(int argc, char** argv) {
    ...
    if (pID == 0) {
        engine.populateLocalRandom(a_sid);
        a_sid %= ATTENDEES_SIZE;
        engine.populateLocalRandom(a_gave_talk);
        // just keep a single bit
        a_gave_talk.mask(1);

        engine.populateLocalRandom(c_sid);
        c_sid %= MAX_STUDENT_ID;
        engine.populateLocalRandom(c_cid);
        c_cid %= MAX_CLASS_ID;
    }
    ...
    EncodedTable<int> Attendees =
        engine.secret_share_table<int>({a_sid, a_gave_talk}, attendee_schema);
    EncodedTable<int> Classes = engine.secret_share_table<int>({c_sid, c_cid}, classes_schema);
    ...
    // We put the subquery in brackets just to scope intermediate tables.
    {
        ...
        Joined.sort(std::vector<std::string>{ENC_TABLE_VALID, "[CountGaveTalk]"}, DESC);
        Joined.head(5);

 ...
 * ```
 *
 * This example demonstrates cdough's vectorized primitives for secret-shared computation.
 */
...
int main(int argc, char** argv) {
    ...
    if (pID == 0) {
        engine.populateLocalRandom(a);
        engine.populateLocalRandom(b);
    ...
    ASharedVector<int> x = engine.secret_share_a(a, 0);
    ASharedVector<int> y = engine.secret_share_a(b, 0);
    ...
    BSharedVector<int> u = engine.secret_share_b(a, 0);
    BSharedVector<int> v = engine.secret_share_b(b, 0);

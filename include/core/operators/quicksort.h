...
void partition(BSharedVector<T, E, Eng>& v, Vector<T>& comparisons, int start, int end,
               Vector<long>& pivots) {
    int i = start;
...
static void quicksort_body(BSharedVector<T, EVector, Engine>& v) {
    ...
    BSharedVector<T, EVector, Engine> temp(N, engine);
    BSharedVector<T, EVector, Engine> comparisons(N, engine);
...
static ElementwisePermutation<EVector, Engine> quicksort(BSharedVector<Share, EVector, Engine>& v,
                                                         SortOrder order, bool no_invert) {
    auto& engine = v.engine;
    ...
    cdough::random::PermutationManager::get()->reserve(v.size(), num_permutations, engine, num_pairs);
    ...
    ElementwisePermutation<EVector, Engine> permutation =

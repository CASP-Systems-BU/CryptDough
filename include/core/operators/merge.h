...
static void compare_swap(BSharedVector<Share, EVector, Engine>& x,
                         BSharedVector<Share, EVector, Engine>& y) {
    auto& engine = x.engine;
    BSharedVector<Share, EVector, Engine> comparisons = (x >= y);
    BSharedVector<Share, EVector, Engine> temp(x.size(), engine);
...
static void odd_even_merge(BSharedVector<Share, EVector, Engine>& v) {
    ...
    BSharedVector<Share, EVector, Engine> c1 = v.slice(0, n - d);
    BSharedVector<Share, EVector, Engine> c2 = v.slice(n - d, n);
    compare_swap(c1, c2);

    int k = 1;
    for (d = n >> 2; d >= 1; d >>= 1) {
        BSharedVector<Share, EVector, Engine> c3 =
            v.slice(d, n - d).alternating_subset_reference(d, d);
        BSharedVector<Share, EVector, Engine> c4 =
            v.slice(d * 2, n).alternating_subset_reference(d, d);
        compare_swap(c3, c4);
    }
}
 ...
 */
template <typename Share, typename EVector, typename Engine>
static void bitonic_merge(std::vector<BSharedVector<Share, EVector, Engine>*> _columns,
                          std::vector<ASharedVector<Share, EVector, Engine>*> _data_a,
                          std::vector<BSharedVector<Share, EVector, Engine>*> _data_b,
    ...
    for (int i = 0; i < rounds; ++i) {
        ...
        std::vector<BSharedVector<Share, EVector, Engine>> x;
        std::vector<BSharedVector<Share, EVector, Engine>> y;
        for (int k = 0; k < _columns.size(); ++k) {
        ...
        BSharedVector<Share, EVector, Engine> bits = compare_rows(x, y, order);
        ...
        if (_data_b.size() > 0) {
            std::vector<BSharedVector<Share, EVector, Engine>> _data_b_1;
            std::vector<BSharedVector<Share, EVector, Engine>> _data_b_2;
            for (int k = 0; k < _data_b.size(); ++k) {
        ...
        if (_data_a.size() > 0) {
            ASharedVector<Share, EVector, Engine> bits_a = bits.b2a_bit();
            std::vector<ASharedVector<Share, EVector, Engine>> _data_a_1;
            std::vector<ASharedVector<Share, EVector, Engine>> _data_a_2;
            for (int k = 0; k < _data_a.size(); ++k) {
...
static void bitonic_merge(std::vector<BSharedVector<Share, EVector, Engine>> _columns,
                          ...
                          std::vector<BSharedVector<Share, EVector, Engine>> _data_b,
                          const std::vector<SortOrder>& order) {
    std::vector<BSharedVector<Share, EVector, Engine>*> res;
    for (BSharedVector<Share, EVector, Engine>& c : _columns) {
        res.push_back(&c);
    }

    std::vector<ASharedVector<Share, EVector, Engine>*> _data_a_;
    for (ASharedVector<Share, EVector, Engine>& c : _columns) {
        res.push_back(&c);
    }

    std::vector<BSharedVector<Share, EVector, Engine>*> _data_b_;
    for (BSharedVector<Share, EVector, Engine>& c : _columns) {
        res.push_back(&c);
    }
 ...
 */
template <typename Share, typename EVector, typename Engine>
static void bitonic_merge(BSharedVector<Share, EVector, Engine>& vec,

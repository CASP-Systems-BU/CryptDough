...
static void distinct(std::vector<BSharedVector<Share, EVector, Engine>*>& keys,
    ...
    BSharedVector<Share, EVector, Engine> first_index = res->slice(0, 1);
    Vector<Share> one(1, 1);
    // enforce single-bit boolean share (all higher bits are 0)
    first_index = engine.public_share_b(one);

    BSharedVector<Share, EVector, Engine> rest = res->slice(1);

    for (int i = 0; i < keys.size(); ++i) {
        // v[0..n-1]
        BSharedVector<Share, EVector, Engine> a = keys[i]->slice(0, keys[i]->size() - 1);
        // v[1..n]
        BSharedVector<Share, EVector, Engine> b = keys[i]->slice(1);
...
static void distinct(std::vector<BSharedVector<Share, EVector, Engine>>& keys,
                     BSharedVector<Share, EVector, Engine>& res) {
    std::vector<BSharedVector<Share, EVector, Engine>*> keys_ptr;

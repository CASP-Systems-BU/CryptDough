...
void count_oblivious_apply_perm(uint64_t size, Engine& engine) {
    using P = cdough::Plaintext_1PC<T, std::vector<T>, cdough::Vector<T>, EVector<T, 1>>;
    P* proto;
    if constexpr (std::is_same_v<T, int8_t>) {
        proto = (P*)engine.worker0->proto_8.get();
    } else if constexpr (std::is_same_v<T, int16_t>) {
        proto = (P*)engine.worker0->proto_16.get();
    } else if constexpr (std::is_same_v<T, int32_t>) {
        proto = (P*)engine.worker0->proto_32.get();
    } else if constexpr (std::is_same_v<T, int64_t>) {
        proto = (P*)engine.worker0->proto_64.get();
    } else if constexpr (std::is_same_v<T, __int128_t>) {
        proto = (P*)engine.worker0->proto_128.get();
    }
...
std::vector<int> gen_perm(size_t size, std::shared_ptr<random::CommonPRG> generator) {
    ...
    cdough::Vector<uint32_t> random(random_buffer_size);
...
void local_apply_perm(Vector<T>& x, const LocalPermutation& permutation, Engine& engine) {
    ...
    engine.execute_parallel_unsafe(x.size(), perm_func);
    // copy back in.
    engine.execute_parallel_unsafe(x.size(), copy_func);
}

/**
 * @brief Overload for both vector and permutation represented as cdough Vector.
 *
 * @tparam T Data type of vector elements.
 * @param x The vector to permute (modified in place).
 * @param permutation The permutation to apply as cdough Vector.
 */
template <typename T, typename Engine>
void local_apply_perm(Vector<T>& x, Vector<int>& permutation, Engine& engine) {
    local_apply_perm(x, permutation.as_std_vector(), engine);
}
...
/**
 * @brief Single-threaded overload for both vector and permutation represented as cdough Vector
 *
 * @tparam T Data type of vector elements.
 * @param x The vector to permute (modified in place).
 * @param permutation The permutation to apply.
...
void local_apply_perm(ElementwisePermutation<EVector, Engine>& x, Vector<int>& permutation) {
    local_apply_perm(x.sharedVector, permutation, x.engine);
}
...
void local_apply_perm(SharedVector<S, E, Eng>& x, P& permutation) {
    local_apply_perm(x.vector, permutation, x.engine);
}
...
/**
 * @brief Overload when both are cdough Vectors.
 *
 * @tparam T Data type of vector elements.
 * @tparam S Permutation data type.
 * @param x The vector to permute.
...
void local_apply_inverse_perm(Vector<T>& x, const Vector<S>& permutation, Engine& engine) {
    local_apply_inverse_perm(x, permutation.as_std_vector(), engine);
}
...
void local_apply_inverse_perm(EVector<T, R>& x, const std::vector<S>& permutation, Engine& engine) {
    for (int r = 0; r < R; r++) {
        local_apply_inverse_perm(x(r), permutation, engine);
    }
}
...
void local_apply_inverse_perm(SharedVector<Share, EVector, Engine>& x,
                              const std::vector<S>& permutation) {
    local_apply_inverse_perm(x.vector, permutation, x.engine);
}
 ...
 */
template <typename EVector, typename Engine>
void local_apply_inverse_perm(ElementwisePermutation<EVector, Engine>& x,
 ...
 */
template <typename Share, typename EVector, typename Engine>
void hm_oblivious_apply_sharded_perm(SharedVector<Share, EVector, Engine>& x,
                                     std::shared_ptr<HMShardedPermutation>& permutation) {
    bool binary = x.encoding;
    auto& engine = x.engine;
    ...
    for (std::set<int> group : groups) {
        ...
        engine.reshare(x.vector, group, binary);
    }
}
 ...
 */
template <typename EVector, typename Engine>
void hm_oblivious_apply_sharded_perm(ElementwisePermutation<EVector, Engine>& x,
                                     std::shared_ptr<HMShardedPermutation>& permutation) {
 ...
 */
template <typename Share, typename EVector, typename Engine>
void hm_oblivious_apply_inverse_sharded_perm(SharedVector<Share, EVector, Engine>& x,
                                             std::shared_ptr<HMShardedPermutation>& permutation) {
    bool binary = x.encoding;
    auto& engine = x.engine;
 ...
 */
template <typename EVector, typename Engine>
void hm_oblivious_apply_inverse_sharded_perm(ElementwisePermutation<EVector, Engine>& x,
                                             std::shared_ptr<HMShardedPermutation>& permutation) {
 ...
 */
template <typename Share, typename EVector, typename Engine>
void permute_and_share(SharedVector<Share, EVector, Engine>& x,
                       std::shared_ptr<DMShardedPermutation<Share>>& perm, int send_party,
                       std::optional<size_t> thread_id = std::nullopt) {
    size_t n = x.size();
    auto& engine = x.engine;

    int rank = engine.getPartyID();
    bool sender = (rank == send_party);
    bool receiver = !sender;
    cdough::Encoding encoding = perm->getEncoding();

    auto [pi, A, B, C] = *(perm->getTuple());

    // get the communicator
    auto comm = engine.workers[thread_id.value_or(0)].getCommunicator();

    if (receiver) {
        // receiver blinds their share
        // delta = [x]_r - A
        if (encoding == cdough::Encoding::BShared) {
            x.vector(0) ^= A;
        } else {
            x.vector(0) -= A;
        }
    ...
    if (sender) {
        ...
        comm->receiveShares(delta, 1);

        // sender permutes delta under pi
        if (!thread_id) {
            local_apply_perm(delta, pi, engine);
        } else {
 ...
 */
template <typename Share, typename EVector, typename Engine>
void permute_and_share_inverse(SharedVector<Share, EVector, Engine>& x,
                               std::shared_ptr<DMShardedPermutation<Share>>& perm, int send_party) {
    size_t n = x.size();
    auto& engine = x.engine;
    ...
    if (receiver) {
        ...
        // receiver sends delta to sender
        engine.comm0()->sendShares(x.vector(0), 1);
    ...
    if (sender) {
        // sender receives delta from receiver
        Vector<Share> delta(n);
        engine.comm0()->receiveShares(delta, 1);
        ...
        local_apply_inverse_perm(delta, pi, engine);
 ...
 */
template <typename Share, typename EVector, typename Engine>
void dm_oblivious_apply_sharded_perm(SharedVector<Share, EVector, Engine>& x,
...
void dm_oblivious_apply_inverse_sharded_perm(
    SharedVector<Share, EVector, Engine>& x,
    std::shared_ptr<DMShardedPermutation<Share>>& permutation) {
 ...
 */
template <typename Share, typename EVector, typename Engine>
void oblivious_apply_sharded_perm(SharedVector<Share, EVector, Engine>& x,
                                  std::shared_ptr<ShardedPermutation>& perm) {
#ifdef INSTRUMENT_APPLYPERM
    count_oblivious_apply_perm<Share>(x.size(), x.engine);
#endif
 ...
 *
 * @tparam EVector Share container type.
 * @param Engine Secure computation engine type.
 * @param x The secret-shared vector to permute.
 * @param permutation The permutation to apply.
 *
 ...
 */
template <typename EVector, typename Engine>
void oblivious_apply_sharded_perm(ElementwisePermutation<EVector, Engine>& x,
 ...
 */
template <typename Share, typename EVector, typename Engine>
void oblivious_apply_inverse_sharded_perm(SharedVector<Share, EVector, Engine>& x,
                                          std::shared_ptr<ShardedPermutation>& perm) {
#ifdef INSTRUMENT_APPLYPERM
    count_oblivious_apply_perm<Share>(x.size(), x.engine);
#endif
 ...
 */
template <typename EVector, typename Engine>
void oblivious_apply_inverse_sharded_perm(ElementwisePermutation<EVector, Engine>& x,
                                          std::shared_ptr<ShardedPermutation>& permutation) {
...
void oblivious_apply_elementwise_perm(SharedVector<Share, EVector, Engine>& x,
                                      ElementwisePermutation<EVectorPerm, Engine>& perm) {
    auto& engine = x.engine;

    // make a deep copy of the permutation
    ElementwisePermutation<EVectorPerm, Engine> permutation(perm);

    // generate a pair of random sharded permutations
    auto [pi_1, pi_2] = PermutationManager::get()->getNextPair<Share, int>(
        x.size(), engine, x.encoding, perm.getEncoding());
 ...
 */
template <typename EVector, typename Engine>
ElementwisePermutation<EVector, Engine> compose_permutations(
    ElementwisePermutation<EVector, Engine>& sigma, ElementwisePermutation<EVector, Engine>& rho) {
    assert(sigma.getEncoding() == rho.getEncoding());
    auto& engine = sigma.sharedVector.engine;

    // generate a random sharded permutation
    std::shared_ptr<ShardedPermutation> pi =
        PermutationManager::get()->getNext<int32_t>(sigma.size(), engine, sigma.getEncoding());
...
static void shuffle(SharedVector<Share, EVector, Engine>& x) {
    ...
    std::shared_ptr<ShardedPermutation> sharded_perm =
        PermutationManager::get()->getNext<Share>(x.size(), x.engine, x.encoding);
    oblivious_apply_sharded_perm(x, sharded_perm);
    x.engine.malicious_check();
}
 ...
 */
template <typename Share, typename EVector, typename Engine>
static void shuffle(std::vector<ASharedVector<Share, EVector, Engine>*> _data_a,
                    std::vector<BSharedVector<Share, EVector, Engine>*> _data_b, size_t size) {
    if constexpr (CONFIDENTIAL_1PC) {
        throw std::logic_error("Shuffle not allowed in CONFIDENTIAL_1PC mode");
    }

    auto& engine = _data_a[0]->engine;
    // generate a random sharded permutation and use it to generate a random
    // elementwise permutation
    std::shared_ptr<ShardedPermutation> sharded_perm =
        PermutationManager::get()->getNext<int32_t>(size, engine, cdough::Encoding::BShared);
    ElementwisePermutation<EVector, Engine> permutation(size, cdough::Encoding::BShared, engine);
    oblivious_apply_sharded_perm(permutation, sharded_perm);

    // apply the shared permutation to all arithmetic columns
    for (ASharedVector<Share, EVector, Engine>* a_column : _data_a) {
        oblivious_apply_elementwise_perm(*a_column, permutation);
    }
    // apply the shared permutation to all binary columns
    for (BSharedVector<Share, EVector, Engine>* b_column : _data_b) {

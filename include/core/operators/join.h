...
TABLE_T TABLE_T::_join(TABLE_T& right, std::vector<std::string> keys, AggregationSpec agg_spec,
    ...
    if (opt.left_outer) {
        if (opt.right_outer) {
            ...
            auto validSlice = concat.asBSharedVector("[##VALID_TEMP]").slice(0, n - 1);
    ...
    if (opt.trim_invalid && !opt.left_outer) {
        ...
        //
        // NOTE: this is the radix sort version. See cdough paper for Quicksort
...
TABLE_T TABLE_T::cartesian_join(TABLE_T& right, std::string key) {
    ...
    TABLE_T output("output", output_cols, nL * nR, this->engine);

...
class Fantastic_4PC : public Protocol<Data, Share, Vector, EVector> {
    ...
    static const auto HASH_RES_ELEMENTS =
        HASH_RES_BYTES / sizeof(Data) + ((HASH_RES_BYTES % sizeof(Data)) != 0);

    static Vector generateHash(const Vector message) {
        // Configure the hash parameters
        Vector hash(HASH_RES_ELEMENTS);

        crypto_generichash((uint8_t*)(&hash[0]), sizeof(Data) * HASH_RES_ELEMENTS,
                           (const uint8_t*)(&message[0]), sizeof(Data) * message.size(), nullptr,
                           0);
     ...
     */
    void malicious_check(const Vector& message, const Vector& hash, const int p1, const int p2) {
        auto generated_hash = generateHash(message);
        if (!generated_hash.same_as(hash)) {
   ...
   public:
    ...
    void multiply_a(const EVector& x, const EVector& y, EVector& z) {
        ...
        Vector mult_recv(size);

        Vector mult_recv_hash(HASH_RES_ELEMENTS);
        auto mult_sent_hash = generateHash(cross_12);

        // // Exchange with next (previous) party
        // this->communicator->exchangeShares(cross_10, mult_recv, +1, +3);
        // // Exchange with opposite party
        // this->communicator->exchangeShares(mult_sent_hash, mult_recv_hash, +2, +2);
        std::vector<Vector> toSend = {cross_10, mult_sent_hash};
        std::vector<Vector> toReceive = {mult_recv, mult_recv_hash};
        std::vector<PartyID> sendParties({+1, +2});
        std::vector<PartyID> receiveParties({+3, +2});
        this->communicator->exchangeShares(toSend, toReceive, sendParties, receiveParties);

        malicious_check(mult_recv, mult_recv_hash, +2, +3);
        ...
        if (this->partyID == 0) {
            ...
            auto mult_sent_hash = generateHash(cross_02);
            // this->communicator->exchangeShares(mult_sent_hash, mult_recv_hash, +1, +1);
            // this->communicator->receiveShares(mult_recv, -1);

            std::vector<Vector> toSend = {mult_sent_hash};
            std::vector<Vector> toReceive = {mult_recv_hash, mult_recv};
            this->communicator->exchangeShares(toSend, toReceive, {+1}, {+1, -1});

            malicious_check(mult_recv, mult_recv_hash, +1, -1);
            ...
            malicious_check(mult_recv, mult_recv_hash, +1, +2);
    ...
    void conv_2d_vectorized_a(const EVector& x, const EVector& y, EVector& z,
        ...
        // Parties generate the cross terms they know
        // ...shared with previous party, randomness excludes next
        auto cross_10 = x(0).conv2DVectorized(y(0), instancesCount, inputHeight, inputWidth,
        ...
        // ...shared with next party, randomness excludes opposite
        auto cross_12 = x(1).conv2DVectorized(y(1), instancesCount, inputHeight, inputWidth,
                                              filterHeight, filterWidth, strideHeight, strideWidth,
                                              paddingHeight, paddingWidth) +
                        x(1).conv2DVectorized(y(2), instancesCount, inputHeight, inputWidth,
                                              filterHeight, filterWidth, strideHeight, strideWidth,
                                              paddingHeight, paddingWidth) +
                        x(2).conv2DVectorized(y(1), instancesCount, inputHeight, inputWidth,
                                              filterHeight, filterWidth, strideHeight, strideWidth,
                                              paddingHeight, paddingWidth) -
                        r_opp;
        ...
        // ...shared with opposite party.
        // We'll randomize below, since it's different per party.
        auto cross_02 = x(0).conv2DVectorized(y(2), instancesCount, inputHeight, inputWidth,
                                              filterHeight, filterWidth, strideHeight, strideWidth,
                                              paddingHeight, paddingWidth) +
                        x(2).conv2DVectorized(y(0), instancesCount, inputHeight, inputWidth,
                                              filterHeight, filterWidth, strideHeight, strideWidth,
                                              paddingHeight, paddingWidth);
    ...
    void matrix_right_multiply_with_column_matrix_vectorized_a(const EVector& x, const EVector& y,
        ...
        // Parties generate the cross terms they know
        // ...shared with previous party, randomness excludes next
        auto cross_10 = x(0).matrixRightMultiplyWithColumnMatrixVectorized(y(0), lhs_rows, lhs_cols,
                                                                           rhs_rows, rhs_cols) +
        ...
        // ...shared with next party, randomness excludes opposite
        auto cross_12 = x(1).matrixRightMultiplyWithColumnMatrixVectorized(y(1), lhs_rows, lhs_cols,
                                                                           rhs_rows, rhs_cols) +
                        x(1).matrixRightMultiplyWithColumnMatrixVectorized(y(2), lhs_rows, lhs_cols,
                                                                           rhs_rows, rhs_cols) +
                        x(2).matrixRightMultiplyWithColumnMatrixVectorized(y(1), lhs_rows, lhs_cols,
                                                                           rhs_rows, rhs_cols) -
                        r_opp;

        // ...shared with opposite party.
        // We'll randomize below, since it's different per party.
        auto cross_02 = x(0).matrixRightMultiplyWithColumnMatrixVectorized(y(2), lhs_rows, lhs_cols,
                                                                           rhs_rows, rhs_cols) +
                        x(2).matrixRightMultiplyWithColumnMatrixVectorized(y(0), lhs_rows, lhs_cols,
                                                                           rhs_rows, rhs_cols);
     ...
     */
    EVector inp_a(const Vector& x, int i, int j, int g, int h) {
        EVector res(x.size());
        long long size = x.size();
        ...
        if (this->partyID == i) {
            ...
            res(2) = x - res(1);
            auto res2Hash = generateHash(res(2));
            this->communicator->sendShares(res2Hash, +2);
        } else if (this->partyID == g) {
            // working as receiver (g in inp)
            // res(0) = [res - r_0]_, res(1) = 0, res(2) = 0
            Vector resHash(HASH_RES_ELEMENTS);
            // this->communicator->receiveShares(res(0), +3);
            // this->communicator->receiveShares(resHash, +2);

            std::vector<Vector> toReceive = {res(0), resHash};
            std::vector<PartyID> receiveParties({+3, +2});
            this->communicator->receiveBroadcast(toReceive, receiveParties);

            malicious_check(res(0), resHash, +2, +3);
     ...
     * @return Shared vector.
     */
    EVector inp_b(const Vector& x, int i, int j, int g, int h) {
        EVector res(x.size());
        long long size = x.size();
    ...
    EVector secret_share_b_internal(const Vector& data, const PartyID& data_party = 0) {
        ...
        if (this->partyID == data_party) {
            ...
            this->communicator->sendShares(boolean_shares[1](2), 1);
            ...
            this->communicator->sendShares(boolean_shares[2](2), 2);
            // Send second shared vector to the successor + 2
            this->communicator->sendShares(boolean_shares[3](0), 3);
            this->communicator->sendShares(boolean_shares[3](1), 3);
            this->communicator->sendShares(boolean_shares[3](2), 3);

            // std::vector<Vector> toSend = {
            //     boolean_shares[1](0), boolean_shares[1](1), boolean_shares[1](2),
            //     boolean_shares[2](0), boolean_shares[2](1), boolean_shares[2](2),
            //     boolean_shares[3](0), boolean_shares[3](1), boolean_shares[3](2)};
            // std::vector<Vector> toReceive;
            // std::vector<PartyID> sendParties({1, 1, 1, 2, 2, 2, 3, 3, 3});
            // this->communicator->exchangeShares(toSend, toReceive, sendParties, {});
            ...
            // std::vector<Vector> toReceive = {s(0), s(1), s(2)};
            // auto rel_data_party = data_party - this->partyID;
            // std::vector<PartyID> receiveParties({rel_data_party, rel_data_party, rel_data_party});
            // this->communicator->receiveBroadcast(toReceive, receiveParties);
            return s;
        }
    }
    ...
    EVector public_share(const Vector& x, const std::set<PartyID>& who_knows) {
        std::cerr << "public_share with who_knows is not implemented yet" << std::endl;
        abort();
    }
    ...
    void reshare(EVector& v, const std::set<int> group, bool binary) {
        ...
        */
        ...
        if (receiver_rel_rank == 3) {  // rank -3
            ...
            auto sharesHash = generateHash(v(1));
            this->communicator->sendShares(sharesHash, receiver_rel_rank);  // share i+2
            ...
            auto sharesHash = generateHash(v(2));
            this->communicator->sendShares(sharesHash, receiver_rel_rank);  // share i
            ...
            auto sharesHash = generateHash(v(2));
            this->communicator->sendShares(sharesHash, receiver_rel_rank);  // share i+1
        } else if (receiver_rel_rank == 0) {
            // this party is the receiver
            std::vector<Vector> received_shares = {v(0), Vector(HASH_RES_ELEMENTS),
                                                   v(2), Vector(HASH_RES_ELEMENTS),
                                                   v(1), Vector(HASH_RES_ELEMENTS)};

...
namespace operators {
    ...
    void oblivious_apply_sharded_perm(
        ...
        std::shared_ptr<cdough::random::ShardedPermutation>& permutation);
...
class ElementwisePermutation {
   ...
   public:
     ...
     */
    ElementwisePermutation(size_t size, Engine& engine)
        : ElementwisePermutation(size, Encoding::AShared, engine) {}
     ...
     */
    ElementwisePermutation(size_t size, Encoding encoding, Engine& engine)
        : sharedVector(size, encoding, engine) {
        Vector<int> identity(size);
        std::iota(identity.begin(), identity.end(), 0);

        // public sharing does the same thing for AShared and BShared
        sharedVector.vector = engine.public_share(identity);
    }
    ...
    ElementwisePermutation(const SharedVector<Share, argEVector, Engine>& v)
        : sharedVector(v.vector, v.encoding, v.engine) {
     ...
     */
    template <typename Share, typename argEVector>
    ElementwisePermutation& operator=(const SharedVector<Share, argEVector, Engine>& other) {
    ...
    ASharedPerm getASharedPerm() {
        ASharedPerm v(size(), sharedVector.engine);
    ...
    BSharedPerm getBSharedPerm() {
        BSharedPerm v(size(), sharedVector.engine);
        v.vector = sharedVector.vector;
        return v;
    }
     ...
     */
    cdough::Encoding getEncoding() { return sharedVector.encoding; }
    ...
    ElementwisePermutation reverse() {
        ...
        ASharedPerm negative_one = sharedVector.engine.public_share_a(negative_one_plaintext)
                                       .repeated_subset_reference(size());
        ASharedPerm additive_constant =
            sharedVector.engine.public_share_a(additive_constant_plaintext)
                .repeated_subset_reference(size());
    ...
    ElementwisePermutation invert() {
        ...
        ElementwisePermutation<EVector, Engine> inverse(size(), this->getEncoding(),
                                                        sharedVector.engine);

        cdough::operators::oblivious_apply_elementwise_perm(inverse.sharedVector, *this);
    ...
    ElementwisePermutation b2a() {
        ...
        if (sharedVector.engine.getNumParties() == 2) {
        ...
        // make a deepcopy
        ElementwisePermutation<EVector, Engine> permutation(*this);

        // generate a random sharded permutation
        auto pi = random::PermutationManager::get()->getNext<int>(size(), sharedVector.engine,
                                                                  Encoding::BShared);

        // shuffle the permutation according to pi
        cdough::operators::oblivious_apply_sharded_perm(permutation.sharedVector, pi);

        // open pi(perm)
        Vector<int> pi_perm = permutation.open();

        ASharedPerm ret = sharedVector.engine.public_share_a(pi_perm);

        // unapply the random permutation
        cdough::operators::oblivious_apply_inverse_sharded_perm(ret, pi);
    ...
    ElementwisePermutation negate() {
        ...
        ASharedPerm v(sharedVector.vector, sharedVector.engine);
        v *= negative_one;
        v += negative_one;

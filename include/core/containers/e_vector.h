...
#include "encoding.h"
#include "vector.h"  // cdough's wrapper for std::vector
...
class EVector {
   ...
   public:
    ...
    EVector& operator=(const cdough::EVector<Other, R, Tag2>& other) {
        for (int i = 0; i < ReplicationNumber; ++i) {
            contents[i] = other.contents[i];
        }
        precision = other.getPrecision();
     ...
     */
    EVector(size_t size, const std::string& _input_file_path) : precision(0) {
     ...
     *
     * NOTE: cdough parties apply operations on EVectors in batches. Each batch corresponds to a
    ...
    define_apply_return_to_replicated(sumPoolingVectorized);
    define_apply_return_to_replicated(matrixSeparateChannels);
    define_apply_return_to_replicated(matrixInterleaveChannels);
    define_apply_return_to_replicated(conv2DLeftVectorization);

    // Functions which take an EVector as input and returns an EVector
    define_apply_input_return_to_replicated(dot_product);
    define_apply_input_return_to_replicated(matrixRightMultiplyWithColumnMatrixVectorized);
    define_apply_input_return_to_replicated(conv2DVectorized);

    // Functions which return a std::vector of EVectors
    define_apply_input_return_to_replicated_std_vector(bit_decomposition);
     ...
     */
    inline Vector<Share>& operator()(int column) { return contents[column]; }
     ...
     *
     */
    inline const Vector<Share>& operator()(int column) const { return contents[column]; }
    ...
    // Friend classes
    template <typename ShareType, typename EVector, typename Engine>

...
namespace operators {
    ...
    static void quicksort_body(BSharedVector<T, Container, Engine>& v);
}  // namespace operators

/**
 * A secret-shared vector with share and container types.
 * @tparam Share Share data type.
 * @tparam Container Share container type.
 ...
 * may vary significantly across protocols. Currently, cdough supports two techniques to construct
...
class SharedVector : public EncodedVector {
   ...
   public:
    ...
    // The secure computation engine creating this shared vector
    Engine& engine;
     ...
     */
    explicit SharedVector(size_t _size, const Encoding& eType, Engine& eng)
        : EncodedVector(eType), vector(_size), engine(eng) {}
     ...
     * @param eng The secure computation engine.
     */
    explicit SharedVector(size_t _size, const std::string& _input_file, const Encoding& eType,
                          Engine& eng)
        : EncodedVector(eType), vector(_size, _input_file), engine(eng) {}
     ...
     * @param _shares The Container whose contents will be pointed at by the SharedVector.
     * @param eType encoding
     * @param eng The secure computation engine.
     */
    explicit SharedVector(const Container& _shares, const Encoding& eType, Engine& eng)
        : EncodedVector(eType), vector(_shares), engine(eng) {}
    ...
    SharedVector(SharedVector&& secretShares) noexcept
        : EncodedVector(secretShares.encoding),
          vector(secretShares.vector),
          engine(secretShares.engine) {}
     ...
     * @param eng The secure computation engine.
     */
    explicit SharedVector(EncodedVector& _shares, Engine& eng)
        : EncodedVector(_shares.encoding), engine(eng) {
    ...
    Vector<T> open() const {
        ...
        if (this->encoding == Encoding::BShared) {
            return engine.open_shares_b(v);
        } else {
            return engine.open_shares_a(v);
        }
    }

    void print() const {
        auto v = open();
        debug::print(v, engine.getPartyID());
    }
    ...
    void zero() { engine.modify_parallel(this->vector, &Container::zero); }
    ...
    SharedVector slice(size_t start, size_t end) {
        return SharedVector(this->asContainer().slice(start, end), encoding, engine);
    }
    ...
    SharedVector subset(size_t start, size_t step, size_t end) {
        return SharedVector(this->asContainer().simple_subset_reference(start, step, end), encoding,
                            engine);
    }

    SharedVector reversed_view() {
        return SharedVector(this->asContainer().directed_subset_reference(-1), encoding, engine);
    }
    ...
    std::unique_ptr<SharedVector> deepcopy() {
        ...
        // Enforce copy
        ev = this->asContainer();
        return std::make_unique<SharedVector>(ev, encoding, engine);
    }

    /**
     * @brief Copy-assignment between shared vector. Copies shares from `other`
     * into `this`. Pass to the engine to enable multithreaded execution.
     *
     * @param other
     * @return SharedVector&
     */
    SharedVector& operator=(const SharedVector& other) {
        ...
        engine.modify_parallel_2arg(this->vector, other.vector, &Container::operator=);
        return *this;
    }

    /**
     * @brief Move-assignment between shared vectors. Takes ownership of other's
     * vector. This is not passed to the engine because no data is copied.
     *
     * @param other
     * @return SharedVector&
     */
    ...
    SharedVector& operator=(const SharedVector<T2, E2, Engine>& other) {
        assert(this->encoding == other.encoding);
        // Calls overloaded Container -> Vector cast-and-copy operators
        engine.modify_parallel_2arg(this->vector, other.vector, &Container::operator=);
        return *this;
    }
     ...
     * @return An Container object with the same contents as `this` shared vector.
     *
     * NOTE: This method is useful for developers who need access to the underlying shares of
     * the SharedVector that are only exposed through the Container. Use it if you are certain
     * about what you are doing.
     */
    Container asContainer() const { return Container(this->vector); }

    /**
     * @brief Get the secure computation engine
     *
     * @return Engine&
     */

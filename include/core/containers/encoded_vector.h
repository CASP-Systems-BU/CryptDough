...
#define binary_op(_op_, type, func, this, y)                              \
    std::unique_ptr<type> operator _op_(const type & y) const {           \
        ...
        auto res = std::make_unique<type>(x_vector.size(), this->engine); \
        this->engine.func(x_vector, y_vector, res->vector);               \
...
#define binary_element_op(_op_, func, T, S)                                                        \
    std::unique_ptr<T> operator _op_(S y) const {                                                  \
        Vector<S> yv({y});                                                                         \
        std::remove_reference_t<decltype(*this)> ypv(this->engine.public_share(yv), this->engine); \
...
#define compound_assignment_op(_op_, func, T)                    \
    inline T operator _op_(const T& y) {                         \
        ...
        assert(x_vector.size() == y_vector.size());              \
        this->engine.func(x_vector, y_vector, x_vector);         \
        return *this;                                            \
    }
...
#define compound_assignment_ptr_op(_op_, func, T)  \
    T& operator _op_(const std::unique_ptr<T> y) { \
        ...
        auto& y_ = y->vector;                      \
        assert(x_.size() == y_.size());            \
        this->engine.func(x_, y_, x_);             \
        return *this;                              \
    }

#define unary_op(_op_, type, func, this)                               \
    std::unique_ptr<type> operator _op_() const {                      \
        auto res = std::make_unique<type>(this->size(), this->engine); \
        this->engine.func(this->vector, res->vector);                  \
        return res;                                                    \
    }
...
#define fn_input_no_return(func, arg, this)              \
    ...
    std::unique_ptr<type> func(T arg) {                  \
        auto res = this->engine.func(this->vector, arg); \
    }

#define svector_reference(TYPE, REF)                          \
    template <typename... T>                                  \
    TYPE REF(T... args) const {                               \
        return TYPE(this->vector.REF(args...), this->engine); \
    }

namespace cdough {
/**
 * A EncodedVector is the main programming abstraction in cdough. All secure operators are applied
 * to encoded vectors, i.e., they take one or more encoded vectors as input and generate one or more
 * encoded vectors as output. cdough computing parties perform secure operations on their encoded
 * vectors as if they were performing plaintext operations on the original (secret) vectors. For
 * example, given two secret vectors v1 and v2, cdough parties can execute the elementwise addition of
 * the secret vectors by simply running v1 + v2 using their local encoded vectors. This way, the cdough
...
class EncodedVector {
   public:
    ...
    // virtual inline cdough::Vector<int>& operator()(int index) = 0;

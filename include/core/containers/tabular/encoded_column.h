...
#define binary_op_downcast(_op_, eType, vType)                                                 \
    std::unique_ptr<EncodedColumn> operator _op_(const EncodedColumn & other) const override { \
        ...
        auto v1 = static_cast<const vType<Share, EVector, Engine>*>(contents.get());           \
        auto v2 = static_cast<const vType<Share, EVector, Engine>*>(other.contents.get());     \
...
#define binary_assignment_downcast(_op_, eType, vType)                                     \
    EncodedColumn& operator _op_(const EncodedColumn & other) override {                   \
        assert(encoding == Encoding::eType && encoding == other.encoding);                 \
        auto v1 = static_cast<vType<Share, EVector, Engine>*>(contents.get());             \
        auto v2 = static_cast<const vType<Share, EVector, Engine>*>(other.contents.get()); \
        *v1 _op_* v2;                                                                      \
        return *this;                                                                      \
    }
...
#define unary_op_downcast(_op_, eType, vType)                                       \
    std::unique_ptr<EncodedColumn> operator _op_() const override {                 \
        ...
        auto v = static_cast<const vType<Share, EVector, Engine>*>(contents.get()); \
...
#define binary_op_either_type(_op_)                                                              \
    std::unique_ptr<EncodedColumn> operator _op_(const EncodedColumn & other) const override {   \
        ...
        if (encoding == AShared) {                                                               \
            auto v1 = static_cast<const ASharedVector<Share, EVector, Engine>*>(contents.get()); \
            auto v2 =                                                                            \
                static_cast<const ASharedVector<Share, EVector, Engine>*>(other.contents.get()); \
            s = std::make_unique<SharedColumn>(*v1 _op_ * v2);                                   \
        } else {                                                                                 \
            auto v1 = static_cast<const BSharedVector<Share, EVector, Engine>*>(contents.get()); \
            auto v2 =                                                                            \
                static_cast<const BSharedVector<Share, EVector, Engine>*>(other.contents.get()); \
            s = std::make_unique<SharedColumn>(*v1 _op_ * v2);                                   \
        }                                                                                        \
        s->encoding = encoding;                                                                  \
        return s;                                                                                \
...
#define binary_op_element(_op_, eType, vType)                                                     \
    std::unique_ptr<EncodedColumn> operator _op_(int64_t other) const override {                  \
        ...
        vType<Share, EVector, Engine> v2(v1->engine.public_share(v2_), v1->engine);               \
...
#define binary_op_element_either_type(_op_)                                                     \
    std::unique_ptr<EncodedColumn> operator _op_(int64_t other) const override {                \
        ...
        if (encoding == AShared) {                                                              \
            auto v = static_cast<const ASharedVector<Share, EVector, Engine>*>(contents.get()); \
            ASharedVector<Share, EVector, Engine> ka = v->engine.public_share_a(k_);            \
            s = std::make_unique<SharedColumn>(*v _op_ ka.cyclic_subset_reference(v->size()));  \
        } else {                                                                                \
            auto v = static_cast<const BSharedVector<Share, EVector, Engine>*>(contents.get()); \
            BSharedVector<Share, EVector, Engine> kb = v->engine.public_share_b(k_);            \
...
#define binary_op_fixed_element(_op_, eType, vType)                            \
    std::unique_ptr<EncodedColumn> operator _op_(int64_t y) const override {   \
        assert(encoding == Encoding::eType);                                   \
        using T = vType<Share, EVector, Engine>;                               \
...
// NOTE: Although these operator templates are defined in namespace cdough, we must now redefine
// them in cdough::relational

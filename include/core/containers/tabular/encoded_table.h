...
#define RESERVED_COLUMNS                                 \
    {ENC_TABLE_PAD,  ENC_TABLE_JOIN_ID, ENC_TABLE_VALID, \
     ENC_TABLE_UNIQ, ENC_TABLE_LABEL}

#ifdef INSTRUMENT_TABLES
#define PRINT_TABLE_INSTRUMENT(...) single_cout(__VA_ARGS__)
#define BEGIN_TABLE_PROFILING()                                                                  \
    stopwatch::profile_comm("other", thread_stopwatch::get_aggregate_comm(engine.getPartyID())); \
...
#define END_TABLE_PROFILING(LABEL)                \
    ...
    stopwatch::profile_comm("table." LABEL,       \
                            thread_stopwatch::get_aggregate_comm(engine.getPartyID()));
...
struct SortInput {
    std::vector<BSharedVector<Share, EVector, Engine>*> columns;
    std::vector<ASharedVector<Share, EVector, Engine>*> data_a;
    std::vector<BSharedVector<Share, EVector, Engine>*> data_b;
...
class EncodedTable {
   ...
   private:
    ...
    EncodedTable& mask(const std::string& mask_column_name, const std::vector<std::string>& keys) {
        ...
        auto shared_single_mask_a = engine.public_share_a(_mask_vec);
        auto shared_single_mask_b = engine.public_share_b(_mask_vec);
   ...
   public:
     ...
     * @param eng The engine to use for this table.
     */
    EncodedTable(const std::string& _tableName, const std::vector<std::string>& _columns, int _rows,
                 Engine& eng)
        : tableName(_tableName), rows(_rows), engine(eng) {
     ...
     */
    EncodedTable(std::vector<std::shared_ptr<EncodedColumn>>&& contents, Engine& eng)
        : engine(eng) {
    ...
    EncodedTable& operator=(const EncodedTable& other) {
        if (this != &other) {
            // Note: engine reference cannot be reassigned, so we assume
            // both tables use the same engine
            assert(&engine == &other.engine);
    ...
    void configureValid() {
        ...
        B one = engine.public_share_b(one_);
    ...
    template <typename T>
    void filter(T&& e) {
        (*this)[ENC_TABLE_VALID] = (*this)[ENC_TABLE_VALID] && std::forward<T>(e);
    }
    ...
    inline void inputSecretShares(const std::string& columnName, const std::string& inputFile) {
        if ((*this)[columnName].encoding == Encoding::BShared) {
            *(B*)((*this)[columnName].contents.get()) = B(rows, inputFile, engine);
        } else {
            *(A*)((*this)[columnName].contents.get()) = A(rows, inputFile, engine);
        }
    }
    ...
    inline void inputCSVTableData(const std::string& _file_path, int _input_party) {
        ...
        if (engine.getPartyID() == _input_party) {
    ...
    inline void outputCSVTableSecretShares(const std::string& _file_path) {
        ...
        if (file.is_open()) {
            ...
            for (auto const& imap : schema) {
                for (int i = 0; i < ((B*)(imap.second.get())->contents.get())
                                        ->asContainer()
     ...
     */
    void addColumns(const std::vector<std::string>& columns_, int rows_) {
    ...
    void addColumn(const std::string& column, const int rows_,
        ...
        if (isBShared(column)) {
            v = std::make_unique<B>(rows_, engine);
        } else {
            v = std::make_unique<A>(rows_, engine);
        }
        if (precision) {
    ...
    EncodedTable& sort(const std::vector<std::pair<std::string, SortOrder>> spec,
        ...
        if constexpr (SPDZ2k_NPC_PROTOCOL) {
            ...
            if (protocol == SortingProtocol::NETWORK) {
                ...
                B id_col = engine.public_share_b(id_vec);
    ...
    EncodedTable& aggregate(const std::vector<std::string>& _keys, AggregationSpec agg_spec,
        ...
        // single_cout("++++ PRE-AGG");
        // print_table(this->open_with_schema(), engine.getPartyID());
...
#ifndef DEBUG_SKIP_EXPENSIVE_TABLE_OPERATIONS
        cdough::aggregators::aggregate(keys_vec, b_agg, a_agg, dir, table_id_vec);
#else
        single_cout("...skipped");
#endif
        ...
        if (opt.mark_valid) {
            if (has_any_aggregation) {
                ...
                if (opt.reverse) {
                    ...
                    short_valid = short_valid && uniq_col.slice(1);
                }
            }
    ...
    void pad_power_of_two(Share pad_value = 0) {
        ...
        if (pad_value != 0) {
            ...
            auto fill_vec = engine.public_share(pad).repeated_subset_reference(pad_size);
    ...
    EncodedTable deepcopy() {
        ...
        auto out = EncodedTable(tableName, col, size(), engine);
    ...
    EncodedTable concatenate(EncodedTable& other, bool power_of_two = false) {
        ...
        TableType new_table(this->name() + "+" + other.name(), new_schema, new_size, engine);
        ...
        B secretSharedOne(1, engine);

        auto ssOne = engine.public_share_b(one);

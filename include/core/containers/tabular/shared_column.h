   ...
   public:
    ...
    explicit SharedColumn(size_t _size, const Encoding& eType) {
        switch (eType) {
            case Encoding::AShared: {
                contents = std::make_unique<ASharedVector<Share, EVector, Engine>>(_size);
                break;
            }
            case Encoding::BShared: {
                contents = std::make_unique<BSharedVector<Share, EVector, Engine>>(_size);
                break;
            }
        }
        encoding = eType;
    ...
    std::unique_ptr<EncodedColumn> deepcopy() override {
        ...
        if (encoding == Encoding::AShared) {
            auto v = static_cast<ASharedVector<Share, EVector, Engine>*>(contents.get());
            auto copy = std::make_unique<ASharedVector<Share, EVector, Engine>>(*v);
            s = std::make_unique<SharedColumn>(std::move(copy));
        } else {
            auto v = static_cast<BSharedVector<Share, EVector, Engine>*>(contents.get());
            auto copy = std::make_unique<BSharedVector<Share, EVector, Engine>>(*v);
            s = std::make_unique<SharedColumn>(std::move(copy));
        }
    ...
    std::unique_ptr<EncodedColumn> operator/(const EncodedColumn& other) const override {
        ...
        std::unique_ptr<BSharedVector<Share, EVector, Engine>> t1;
        std::unique_ptr<BSharedVector<Share, EVector, Engine>> t2;

        const BSharedVector<Share, EVector, Engine>* v1;
        const BSharedVector<Share, EVector, Engine>* v2;

        if (encoding == AShared) {
            t1 = static_cast<const ASharedVector<Share, EVector, Engine>*>(contents.get())->a2b();
            v1 = t1.get();
        } else {
            v1 = static_cast<const BSharedVector<Share, EVector, Engine>*>(contents.get());
        }

        if (other.encoding == AShared) {
            t2 = static_cast<const ASharedVector<Share, EVector, Engine>*>(other.contents.get())
                     ->a2b();
            v2 = t2.get();
        } else {
            v2 = static_cast<const BSharedVector<Share, EVector, Engine>*>(other.contents.get());
        }

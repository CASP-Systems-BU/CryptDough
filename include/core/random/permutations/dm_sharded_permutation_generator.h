   ...
   public:
    ...
    DMShardedPermutation(size_t n)
        : perm(std::make_shared<dm_perm_t>(n, n, n, n)),
          encoding(cdough::Encoding::BShared),
          common_prg(nullptr),
     ...
     */
    DMShardedPermutation(size_t n, cdough::Encoding _encoding)
        : perm(std::make_shared<dm_perm_t>(n, n, n, n)),
          encoding(_encoding),
          common_prg(nullptr),
          has_common_prg(false) {}
     ...
     * @param _encoding The encoding type of the permutation correlation.
     */
    DMShardedPermutation(dm_perm_t _perm, cdough::Encoding _encoding)
     ...
     */
    cdough::Encoding getEncoding() { return encoding; }
...
std::shared_ptr<DMShardedPermutation<T>> dm_perm_convert_b2a(
    std::shared_ptr<DMShardedPermutation<T>> perm, Engine& engine) {
    auto [pi, A, B, C] = *(perm->getTuple());

    // create BSharedVectors
    typename Engine::template BSVector<T> B_binary(perm->size(), engine);
    typename Engine::template BSVector<T> C_binary(perm->size(), engine);
...
#else
    auto B_arithmetic = B_binary.insecure_b2a();
    auto C_arithmetic = C_binary.insecure_b2a();
#endif
...
class DMShardedPermutationGenerator : public ShardedPermutationGenerator {
   ...
   public:
    ...
    void assertCorrelated(std::shared_ptr<DMShardedPermutation<T>>& perm) {
        ...
        bool is_a_shared = perm->getEncoding() == cdough::Encoding::AShared;
        ...
        if (this->rank == 0) {
            ...
            // check correctness - 0
            cdough::operators::local_apply_perm_single_threaded(pi_A_0, pi_0);
            ...
            // check correctness - 1
            cdough::operators::local_apply_perm_single_threaded(pi_A_1, pi_1);

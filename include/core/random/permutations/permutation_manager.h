...
class PermutationManager {
   ...
   public:
     ...
     * @param engine The engine used to generate the permutations.
     * @param num_pairs (2PC only) how many pairs of correlated permutations to reserve.
     */
    template <typename Engine>
    void reserve(size_t size_permutation, size_t num_permutations, Engine& engine,
                 size_t num_pairs = 0) {
        ...
        auto generator =
            engine.rand0()->template getCorrelation<__int128_t, ShardedPermutationGenerator>();
        ...
        engine.template generate_permutations<__int128_t>(ret);
     ...
     * @param engine The engine used to get the permutation.
    ...
    std::shared_ptr<ShardedPermutation> getNext(
        size_t size_permutation, Engine& engine,
        std::optional<cdough::Encoding> dm_encoding = cdough::Encoding::BShared) {
        if (engine.getNumParties() == 2) {
            stopwatch::profile_preprocessing();
        }
        ...
        if (size() == 0) {
            ...
            if (size_permutation != stored_size) {
                ...
                reserve(size_permutation, 1, engine, 0);
            }
        ...
        if (engine.getNumParties() == 2) {
            ...
            if (dm_encoding.value() == cdough::Encoding::AShared) {
                next = dm_perm_convert_b2a(converted, engine);
            } else {
                next = converted;
            }
        }
     ...
     * @param size_permutation The size of the permutation to get.
     * @param engine The engine used to get the permutations.
    ...
    std::pair<std::shared_ptr<ShardedPermutation>, std::shared_ptr<ShardedPermutation>> getNextPair(
        size_t size_permutation, Engine& engine,
        std::optional<cdough::Encoding> dm_encoding_1 = cdough::Encoding::BShared,
        std::optional<cdough::Encoding> dm_encoding_2 = cdough::Encoding::BShared) {
        if (engine.getNumParties() == 2) {
            stopwatch::profile_preprocessing();
        }
        ...
        if (size_pairs() == 0) {
            ...
            reserve(size_permutation, 0, engine, 1);
        }
        ...
        if (dm_encoding_1.value() == cdough::Encoding::AShared) {
            result_1 = dm_perm_convert_b2a(converted_1, engine);
        }

        if (dm_encoding_2.value() == cdough::Encoding::AShared) {
            result_2 = dm_perm_convert_b2a(converted_2, engine);
        }

...
class CommonPRG {
   ...
   public:
    ...
    void getNext(Vector<NTL::GF2E>& nums) {
        int poly_bytes = math::div_ceil(NTL::GF2E::degree(), 8L);
...
class CommonPRGManager {
   ...
   public:
    /**
     * @brief Construct an empty CommonPRG manager with one party (i.e., local PRG only.)
     *
     */
    CommonPRGManager() { CommonPRGManager(1); }
    ...
    CommonPRGManager(int _num_parties) : num_parties(_num_parties) {
        ...
        // CommonPRG with oneself is the local PRG
        common_prgs[0] = std::make_shared<CommonPRG>(CommonPRG());
    }

...
class AuthTripleGeneratorBase : public CorrelationGenerator<std::tuple<TensorT, TensorT, TensorT>> {
    using S = TensorT;
   ...
   public:
     ...
     */
    AuthTripleGeneratorBase(PartyID rank) : CorrelationGenerator<triple_t>(rank) {}
 ...
 */
template <typename T, typename TensorT>
class DummyAuthTripleGenerator : public AuthTripleGeneratorBase<T, TensorT> {
   ...
   public:
     ...
     */
    DummyAuthTripleGenerator(int partiesNum, const T& keyShare, PartyID rank,
                             std::shared_ptr<CommonPRG> localPRG,
                             std::shared_ptr<ZeroSharingGenerator> zeroSharingGenerator,
                             Communicator* comm)
        : AuthTripleGeneratorBase<T, TensorT>(rank),
    ...
    void assertCorrelated(const triple_t& bt) override {
        ...
        auto b_opened =
            testing::OpenAdditiveShares(b(0), this->rank, this->partiesNum_, this->comm_);
        auto c_opened =
            testing::OpenAdditiveShares(c(0), this->rank, this->partiesNum_, this->comm_);
        ...
        auto bm_opened =
            testing::OpenAdditiveShares(b(1), this->rank, this->partiesNum_, this->comm_);
        auto cm_opened =
            testing::OpenAdditiveShares(c(1), this->rank, this->partiesNum_, this->comm_);
 ...
 */
template <typename T, typename TensorT>
class ZeroAuthTripleGenerator : public AuthTripleGeneratorBase<T, TensorT> {
    using S = TensorT;
    using triple_t = std::tuple<S, S, S>;
    using vec_t = Vector<T>;
   ...
   public:
     ...
     * @param rank The rank of this party.
     */
    ZeroAuthTripleGenerator(PartyID rank) : AuthTripleGeneratorBase<T, TensorT>(rank) {}

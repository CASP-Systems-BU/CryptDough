...
/**
 * A parent class representing the engine's fundamental unit of single-threaded
 * computation. For a single call into the engine (e.g. MPC functionality),
...
class Task_1_ref : public Task {
   ...
   public:
    Task_1_ref(const In& _x, Ret& _res, const size_t _start, const size_t _end,
               const ssize_t _batch_size, const bool& _batch_input, std::function<void(In&, Ret&)> _func)
        : Task(_start, _end, _batch_size), x(_x), res(_res), batch_input(_batch_input), func(_func) {}
...
/**
 * A Task which takes two inputs and returns 2 outputs via passed reference.
 *
 * @tparam In The type of both inputs.
 * @tparam Ret The type of the results.
 */
template <typename In, typename Ret, typename... T>
class Task_2_ref_2_rtr : public Task {
    In x, y;
    Ret res1, res2;

    // the function to execute on a range of the input
    std::function<void(In&, In&, Ret&, Ret&)> func;

   public:
    Task_2_ref_2_rtr(const In& _x, const In& _y, Ret& _res1, Ret& _res2, const size_t _start, const size_t _end,
               const ssize_t _batch_size, std::function<void(In&, In&, Ret&, Ret&)> _func)
        : Task(_start, _end, _batch_size), x(_x), y(_y), res1(_res1), res2(_res2), func(_func) {}

    void sub_execute(size_t start, size_t end) override {
        x.set_batch(start, end);
        y.set_batch(start, end);
        res1.set_batch(start, end);
        res2.set_batch(start, end);
        func(x, y, res1, res2);
    }
};
...
class Task_2_Agg_ref : public Task {
   ...
   public:
    Task_2_Agg_ref(const In& _x, const In& _y, Ret& _res, const size_t _start, const size_t _end,
                   const ssize_t _batch_size, size_t _agg_size,
...
/**
 * A Task which takes two inputs and returns via passed reference, but where
 * only the first input is batched; the second input is unbatched.
 * The output is batched to match the ratio of input to output sizes.
 *
 * @tparam In The type of both inputs.
 * @tparam Ret The type of the result.
 */
template <typename In, typename Ret, typename... T>
class Task_2_Matrix_ref : public Task {
    In x, y;
    Ret res;
   ...
   public:
    Task_2_Matrix_ref(const In& _x, const In& _y, Ret& _res, const size_t _start, const size_t _end,
                      const ssize_t _batch_size, size_t _input_size, size_t _output_size,
                      std::function<void(In&, In&, Ret&)> _func)
        : Task(_start, _end, _batch_size),
          x(_x),
          y(_y),
    ...
    void sub_execute(size_t start, size_t end) override {
        x.set_batch(start, end);  // first input is batched normally.
        y.reset_batch();          // y is unbatched
        // output is batched but batch size is adjusted to match the ratio of input to output sizes.
        res.set_batch(start / input_size * output_size, end / input_size * output_size);
        func(x, y, res);
    }
};

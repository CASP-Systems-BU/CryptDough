...
class Worker {
   public:
    Worker(int rank, std::shared_ptr<std::barrier<>> barrier, int wid)
        : terminate_(false),
          b_(barrier),
    ...
    bool malicious_check() {
        bool ok = proto_8->malicious_check();
        ok &= proto_16->malicious_check();
        ok &= proto_32->malicious_check();
        ok &= proto_64->malicious_check();
        ok &= proto_128->malicious_check();
        return ok;
    }
    ...
    cdough::random::RandomnessManager* getRandManager() const {
        assert(rand_ != nullptr);
        return this->rand_.get();
    }

    cdough::Communicator* getCommunicator() const {
    ...
    void init_proto(PF& pf) {
        ...
        auto p = pf.template create<T>(workerId, c, r);
    ...
    void attach(std::unique_ptr<Communicator>&& c,
                std::unique_ptr<cdough::random::RandomnessManager>&& r) {
        assert(c != nullptr && r != nullptr);
        comm_ = std::move(c);
        rand_ = std::move(r);
    }
     ...
     * @param r
     */
    void attach_rand(std::unique_ptr<cdough::random::RandomnessManager>&& r) {
        assert(r != nullptr);
        rand_ = std::move(r);
    }

    // TODO: make this cleaner - singleton template types?
    // Ideally should be private, somehow, but engine needs access to
   ...
   private:
    ...
    // ...randomness manager...
    std::unique_ptr<cdough::random::RandomnessManager> rand_;
    // ...and task queue.

The `scripts/` directory aggregates helper scripts for setting up CryptDough's dependencies, running experiments, and visualizing results.

Sub-directories:
- `comm/` - communication scripts, including WAN simulation
- `competitors/` - scripts for running competitors in the CryptDough paper
...
Top-level scripts:
- `setup.sh` – One-shot installer for CryptDough’s external dependencies; calls:
